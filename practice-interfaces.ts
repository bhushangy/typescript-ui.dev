...
function compileFruitReview(fruit: MyFruit): string {
  let review: string = `This ${fruit.name} has a nice ${fruit.color} color to it.`;
  if (fruit.sweetness < 50) {
    review += ' It could be a little sweeter.';
  } else {
...
interface Apple extends MyFruit {
  variety: string;
}
...
async function fetchFruitOrUseCache(id: string | number) {
  if (fruitCache[id]) {
    return fruitCache[id];
  }
  const response = await fetch(`https://example.com/fruit/${id}`);
  const fruit: MyFruit = await response.json();
  fruitCache[id] = fruit;
  return fruit;
}
